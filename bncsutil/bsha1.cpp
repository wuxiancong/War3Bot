/**
 * @file bsha1.cpp
 * @brief Battle.net "Broken" SHA-1 算法实现
 *
 * 作用:
 * 此文件实现了暴雪早期游戏中使用的非标准 SHA-1 算法（通常被称为 Broken SHA-1）。
 * 它主要用于 StarCraft (星际争霸 1), Diablo II (暗黑破坏神 2) 和 WarCraft II BNE 的
 * 版本验证 (CheckRevision) 和 CD-Key 哈希计算。
 *
 * 注意:
 * 这与标准的 SHA-1 不同，不可混用。对于 WarCraft III，暴雪使用了标准的 SHA-1 (sha1.cpp)。
 */
