...
#### 2. 为什么 [test] 能过，而 [bot] 挂了？

*   **[test] (真人)**：
    *   服务端发的是 ASCII `"91..."`。
    *   虽然是 ASCII，但它被解析器读取时，字符串的 `\0` 结束符位置、数组的长度，恰好让指针最后**对齐**了。这依然是一个概率极小的巧合（幸存者偏差）。

*   **[bot] (机器人)**：
    *   服务端发的是 ASCII Hex String（例如长度 200 字节）。
    *   但原本的 Binary 数据只有 100 字节。
    *   **解码器**：把这 200 字节 ASCII 当作二进制解密了。
    *   **解析器**：
        *   调用 `ReadString` 读取地图名 -> 读到某个偶然的 `00` 停下。
        *   调用 `ReadString` 读取创建者 -> 读到另一个 `00` 停下。
        *   ... 一路读完。
    *   **最终检查**：
        *   解析器觉得：“按照规则，我读完了所有字段，我走了 **80 个步长**”。
        *   但是缓冲区总长度是 **200 个步长**（因为发的是 Hex String，长度翻倍了）。
        *   **比较**：`80 != 200`。
        *   **判定**：数据流未耗尽（Data Leftover），数据包结构错乱。
        *   **结果**：`jne` 跳转，返回 `0` (False)。

### 修正后的完整逻辑链

1.  **收包**：收到错误的 ASCII Hex String。
2.  **解码 (`6F650820`)**：
    *   **令人意外地**返回了 `1` (True)。说明头部校验比较宽松，或者运气好。
3.  **解析 (`6F654510`)**：
    *   程序按部就班地读取了地图名、创建者等信息（此时内存里已经填入了这些乱码数据）。
4.  **完整性校验 (`6F654672`)**：
    *   检查 `CurrentPtr == EndPtr`。
    *   由于 Hex String 长度通常是 Binary 的 **2倍**，解析器读完正常逻辑所需的字节后，**缓冲区里还剩下一大半数据没读**。
    *   **校验失败！**
5.  **回滚/丢弃**：
    *   函数返回 `0`。
    *   上层函数（UI刷新函数）看到返回 `0`，认为数据无效，**丢弃**了刚刚读取的所有信息。
    *   **现象**：右侧面板空白。
...
**bot 房间挂掉，不是因为“解密失败”，而是因为“长度不匹配（Parsing Mismatch）”。**
