/**
 * @file nls.cpp
 * @brief NLS (SRP) 认证协议实现文件
 *
 * 作用：
 * 此文件实现了 Battle.net 登录流程中最核心的 NLS (Nifty Lightweight Protocol) 协议。
 * 这是一种基于 SRP (Secure Remote Password) 的安全认证机制。
 *
 * 核心功能：
 * 1. 依赖 GMP (GNU Multiple Precision Arithmetic Library) 进行大数运算。
 * 2. 处理公钥 (A) 和私钥的生成。
 * 3. 计算客户端证明 (M1) 以向服务器证明密码正确。
 * 4. 验证服务器证明 (M2) 以防止中间人攻击。
 * 5. 生成会话密钥 (Session Key/K) 用于后续通信加密。
 */
...
struct _nls {
    ...
    gmp_randstate_t rand; // 随机数状态

    char *A;  // 公钥
    char *S;  // 共享密钥 (Secret)
    char *K;  // 会话密钥 (Hashed Secret)
    char *M1; // 客户端证明
    char *M2; // 服务器证明
};
...
/*
 * 原始大整数常量 I (SHA1(g) ^ SHA1(N))
 */
const uint8_t bncsutil_NLS_I[] = {
    0x6c, 0xe, 0x97, 0xed, 0xa, 0xf9, 0x6b, 0xab, 0xb1, 0x58, 0x89, 0xeb,
    0x8b, 0xba, 0x25, 0xa4, 0xf0, 0x8c, 0x1, 0xf8
};

/*
 * 服务器签名常量 n (1024-bit modulus for signature check)
 */
const uint8_t bncsutil_NLS_sig_n[] = {
...
MEXP(void) nls_free(nls_t *nls) {
    ...
    if (nls->S) free(nls->S);
    if (nls->K) free(nls->K);
    if (nls->M1) free(nls->M1);
    if (nls->M2) free(nls->M2);
...
MEXP(unsigned long) nls_account_create(nls_t *nls, char *buf, unsigned long bufSize) {
    ...
    memcpy(buf + 64, nls->username, nls->username_len);
    buf[64 + nls->username_len] = '\0';
...
MEXP(unsigned long) nls_account_logon(nls_t *nls, char *buf, unsigned long bufSize) {
    ...
    nls_get_A(nls, buf);
    memcpy(buf + 32, nls->username, nls->username_len);
    buf[32 + nls->username_len] = '\0';
