...
; [注意] 即使是 Bot 发送的 ASCII Hex，这里依然返回了 1 (成功)，说明头部检查较宽松或巧合
...
; ------------------------------------------------------------------------
; [TEST 房间 (真人)] 
; 服务端发送了 ASCII "91..."，解析器读取字段后，指针恰好（巧合地）对齐到了末尾。
; 结果: 相等 (Equal) -> JNE 不跳转 -> 继续执行 -> 解析成功。
; ------------------------------------------------------------------------
; [BOT 房间 (机器人)]
; 服务端发送了 ASCII Hex String (长度是 Binary 的 2 倍)。
; 解析器按照逻辑读取了地图名等字段，但缓冲区后面还剩下大量未读数据 (Garbage/Leftover)。
; 结果: 不相等 (Not Equal) -> JNE 跳转触发 -> 进入错误处理。
; ------------------------------------------------------------------------
