...
### 2. 为什么 GameUI 无法获取且无法申请内存？

`6F3A699C call game.6F300710` 获取 `GameUI` 失败。让我们看 `6F300710`：

```assembly
6F300736  | 833D 804FAB6F 00 | cmp dword ptr ds:[6FAB4F80],0 ; 检查全局单例 GameUI 指针
6F30073D  | 75 35            | jne game.6F300774             ; 如果存在，直接返回
...
6F300745  | 68 AF1A0000      | push 1AAF                     ; 内存大小 0x1AAF
6F30074A  | 68 585B936F      | push game.6F935B58            ; ".\\CGameUI.cpp" 字符串
...
6F300754  | E8 59AE3E00      | call <JMP.&Ordinal#401>       ; Storm.AllocateMemory (申请内存)
```

如果这里失败（或者返回空），通常不是因为物理内存不足，而是因为 **前提条件未满足**。

**核心原因：**
`GameUI` 是在地图加载成功、游戏世界初始化之后才创建的。
目前的现象是：
1.  全局状态检查 (`0x278`) 失败 -> 认为游戏未开始。
2.  `GameUI` 指针为空 -> 界面未初始化。

这说明 **地图加载过程崩溃了** 或者 **初始化被提前终止了**。
